...
const SyllabusFolderScreen = ({ route, navigation }) => {
  const { parentId = 'null', title = 'Syllabus', level = 'group' } = route.params || {};
  ...
  const handleCreate = async () => {
    ...
    try {
      await createFolder({
        ...
        category: 'SYLLABUS',
        isFinal: level === 'stage', // If creating under Group, it becomes Stage (which holds PDFs)
      }).unwrap();
      setNewFolderName('');
    } catch (err) {
  ...
  const handleDelete = (id) => {
    Alert.alert(
      'Delete Folder', 
      'Are you sure? Deleting this folder will permanently remove all subfolders and syllabus files inside.', 
      [
  ...
  const renderItem = ({ item }) => {
    return (
      <TouchableOpacity 
        ...
        onPress={() => {
          if (level === 'group') {
            navigation.push('SyllabusFolder', { 
              parentId: item._id, 
              title: item.name, 
              level: 'stage' 
            });
          } else {
            navigation.push('SyllabusContent', { 
              subjectId: item._id, 
      ...
      >
        <View style={[styles.iconBox, { backgroundColor: themeColors.iconBg }]}>
          <MaterialCommunityIcons 
            name={level === 'group' ? 'folder-table-outline' : 'folder-open-outline'} 
            size={26} 
        ...
        <View style={styles.infoBox}>
          ...
          <Text style={[styles.itemSub, { color: themeColors.textMuted }]}>
            {level === 'group' ? 'Syllabus Group' : 'Exam Stage'}
          </Text>
        </View>
      ...
      <View style={[styles.header, { backgroundColor: themeColors.headerBg, borderBottomColor: themeColors.border }]}>
        ...
        <View>
          ...
          <Text style={[styles.headerSubtitle, { color: themeColors.primary }]}>
            {level === 'group' ? 'SYLLABUS GROUPS' : 'EXAM STAGES'}
          </Text>
        </View>
      </View>
      ...
      {isAdmin && (
        <View style={styles.adminCreateBox}>
          <TextInput 
            ...
            placeholder={`Add new ${level === 'group' ? 'group (e.g. गट_अ)' : 'stage (e.g. Prelims)'}...`}
        ...
        <FlatList
          ...
          ListEmptyComponent={() => (
            <View style={styles.empty}>
              ...
              <Text style={[styles.emptyText, { color: themeColors.textMuted }]}>No syllabus folders created yet</Text>
              {isAdmin && (
...
export default SyllabusFolderScreen;
