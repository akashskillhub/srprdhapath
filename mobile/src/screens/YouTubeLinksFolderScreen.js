...
const YouTubeLinksFolderScreen = ({ route, navigation }) => {
  const { parentId = 'null', title = 'Important Links', level = 'root' } = route.params || {};
  ...
  const themeColors = {
    ...
    iconBg: isDarkMode ? 'rgba(255, 0, 0, 0.15)' : 'rgba(255, 0, 0, 0.05)',
  ...
  const handleDelete = (id) => {
    Alert.alert(
      'Delete Folder', 
      'Are you sure? Deleting this folder will permanently remove all links and subfolders inside.', 
      [
      ...
      <TouchableOpacity 
        style={[
          ...
          { 
            ...
            shadowColor: isDarkMode ? '#000000' : 'rgba(255, 0, 0, 0.2)'
          }
        ]}
        activeOpacity={0.8}
        onPress={() => {
          if (level === 'root') {
            navigation.push('YouTubeLinksFolder', { 
              parentId: item._id, 
              title: item.name, 
              level: 'subfolder' 
            });
          } else {
            navigation.push('YouTubeLinksContent', { 
              folderId: item._id, 
      ...
      >
        <View style={[styles.iconBox, { backgroundColor: themeColors.iconBg }]}>
          <MaterialCommunityIcons 
            name={level === 'root' ? 'youtube-subscription' : 'folder-play-outline'} 
            size={26} 
        ...
        <View style={styles.infoBox}>
          ...
          <Text style={[styles.itemSub, { color: themeColors.textMuted }]}>
            {level === 'root' ? 'Platform/Category' : 'Playlist/Section'}
          </Text>
        </View>
        ...
        {isAdmin ? (
          <View style={styles.adminActions}>
            ...
            <TouchableOpacity 
              ...
              style={[styles.actionBtn, { backgroundColor: 'rgba(255, 0, 0, 0.1)', marginLeft: 10 }]}
            >
      ...
      <View style={[styles.header, { backgroundColor: themeColors.headerBg, borderBottomColor: themeColors.border }]}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={[styles.headerSubtitle, { color: themeColors.primary }]}>
            {level === 'root' ? 'IMPORTANT LINKS' : 'SUB CATEGORIES'}
          </Text>
        </View>
      </View>
      ...
      {isAdmin && (
        <View style={styles.adminCreateBox}>
          <TextInput 
            ...
            placeholder={`Add new ${level === 'root' ? 'category (e.g. YouTube)' : 'subfolder (e.g. 1_Documentary)'}...`}
        ...
        <FlatList
          ...
          ListEmptyComponent={() => (
            <View style={styles.empty}>
              <MaterialCommunityIcons name="youtube-tv" size={80} color={themeColors.border} />
              <Text style={[styles.emptyText, { color: themeColors.textMuted }]}>No folders created yet</Text>
              {isAdmin && (
                <Text style={{ color: themeColors.textMuted, fontSize: 13, marginTop: 5 }}>
                  Use the input field above to add a new folder
                </Text>
              )}
            </View>
          )}
...
export default YouTubeLinksFolderScreen;
